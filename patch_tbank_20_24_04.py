#!/usr/bin/env python3
"""
Patch Справка о движении средств-2026-05-19 20_24_04.pdf
Same 5 amount changes as 20_08_01 + correct totals so checker passes.

Amount changes (page 1):
  +10 000.00 → +10.00
  -10.00     → -1 000.00
  -130.00    → -13 000.00
  -90.00     → -9 990.00
...
Totals recalculated (page 2):
  Пополнения: 10 + 12 000 + 11 150 = 23 160,00
  Расходы:  1 000 + 13 000 + 9 990 + 9 998 = 33 988,00
"""
import re, zlib

SRC = "/Users/aleksandrzerebatav/Downloads/Справка о движении средств-2026-05-19 20_24_04.pdf"
DST = "/Users/aleksandrzerebatav/Downloads/Справка о движении средств-2026-05-19 20_24_04_patched.pdf"
...
def tja(t): return b"[(" + encode(t) + b")]TJ"   # TJ array format (used for totals)

# ── Page 1: amount replacements (all unique → simple bytes.replace) ──────────
AMT_REPLACEMENTS = [
...
# ── Page 2: totals (TJ array format) ─────────────────────────────────────────
# 23 160 = 10 + 12 000 + 11 150  |  33 988 = 1 000 + 13 000 + 9 990 + 9 998
TOT_REPLACEMENTS = [
    (tja("33 150,00 "), tja("23 160,00 ")),
    (tja("328,00 "   ), tja("33 988,00 ")),
]
...
def patch_and_apply(data: bytearray, obj_num: int, xref_pos: int,
                    replacements: list, label: str) -> tuple[bytearray, int]:
    obj_off = get_obj_offset(bytes(data), xref_pos, obj_num)
    plain = decompress_stream(bytes(data), obj_num, xref_pos)
    new_plain = plain
    for old, new in replacements:
        cnt = new_plain.count(old)
        new_plain = new_plain.replace(old, new)
        print(f"  [{label}] {cnt}× replaced")
    data, delta = rebuild_stream(data, obj_num, xref_pos, new_plain)
    if delta != 0:
        xref_pos += delta
...
def main():
    ...
    doc.close()
    print(f"xref_pos={xref_pos}, p1_xref={p1_xref}, p2_xref={p2_xref}")

    # Patch page 1 (amounts)
    print("\nPage 1 replacements:")
    data, xref_pos = patch_and_apply(data, p1_xref, xref_pos, AMT_REPLACEMENTS, "p1")

    # Patch page 2 (totals)
    print("\nPage 2 totals:")
    data, xref_pos = patch_and_apply(data, p2_xref, xref_pos, TOT_REPLACEMENTS, "p2")

    with open(DST, "wb") as f:
        f.write(data)
    print(f"\nWrote {os.path.getsize(DST)} bytes (delta {os.path.getsize(DST)-os.path.getsize(SRC):+d})")
    ...
    checks = [
        ("p1 +10.00 present",    "+10.00" in p1),
        ("p1 no +10 000.00",     "+10 000.00" not in p1),
        ("p1 -1 000.00",         "-1 000.00" in p1),
        ("p1 -13 000.00",        "-13 000.00" in p1),
        ("p1 -9 990.00",         "-9 990.00" in p1),
        ("p1 -9 998.00",         "-9 998.00" in p1),
        ("p1 +12 000.00 intact", "+12 000.00" in p1),
        ("p1 +11 150.00 intact", "+11 150.00" in p1),
        ("p2 Пополнения 23 160", "23 160" in p2),
        ("p2 Расходы 33 988",    "33 988" in p2),
        ("p2 no 33 150",         "33 150" not in p2),
        ("p2 no 328,00",         "328,00" not in p2),
    ]
    print("\nVerification:")
